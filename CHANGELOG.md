...
- The location preview now draws the region's actual (simplified) BORDER instead
  ...
  easier to recognise. Borders come from borders.json served on rfsat.com beside
  index.json (fetched + cached like the catalogue; regions without a border fall
  back to their bbox). Generated by the new build_borders.py tool from Natural
  Earth admin-0/admin-1 data.
- The preview can now always zoom out to the full Europe view (pinch or
  double-tap), for every region and sub-region, to place it in Europe.
- Region list overhaul:
  * Countries that have sub-regions are marked "(regions)" and expand on tap.
  * Countries with NO sub-regions (e.g. Greece) open their info window directly
    on tap instead of expanding an empty list.
  * Each country row shows the TOTAL download size (all sub-regions) and a "Get
    all" button that downloads every sub-region at once (concurrently, each with
    its own progress bar). Standalone countries show a "Get" button.
  * Sub-region rows have their own narrow download buttons (Get / Upd / Del) so
    they don't inflate row height.
- borders.json currently covers 125/155 regions; the rest (most French old-regions,
  UK constituent countries, a few micro-states) fall back to bbox until their
  name mappings are added to build_borders.py.
